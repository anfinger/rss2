...
########################################################
# XML Export alle Reisen nach Termin Winter 2026/27    #
########################################################
def reiseuebersichtwinter(request):
    ...
    for i in range(len(termine)):
      ...
      if u'Blauf' in termine[i].Reiseziel and blau != True:
        termine[i] = termine[i]._replace(Termin = 'Okt. 26')
        blau = True
      ...
      if u'Dankesch' in termine[i].Reiseziel and danke != True:
        termine[i] = termine[i]._replace(Termin = 'Nov. 26 & Jan. 27')
        danke = True
...
def reiseterminuebersicht(request):
    ...
    cursor.execute("""
                    ...
                    ON
                      (reisen_reise.reiseID = reisen_reisekatalogzugehoerigkeit.reise_id_id)
                    WHERE
                      reisen_reisekatalogzugehoerigkeit.katalog_id_id = '54b33aabdddc4c41a3fa623e21134818'
                    AND
    ...
    cursor.execute("""
                     ...
                     FROM
                       (
                         (
                           SELECT DISTINCT
                             reisen_reise.reiseID,
                             reisen_reise.individualbuchbar,
                             reisen_reise.neu,
                             'Kategorie' AS kategorietyp,
                             reisen_kategorie.kategorie,
                             reisen_reise.korrektur_bemerkung_intern AS intern,
                             reisen_reise.titel AS Reise,
                             (TO_DAYS(reisen_reisetermine.datum_ende)-TO_DAYS(reisen_reisetermine.datum_beginn)+1) AS Tage,
                             katalogseite,
                             anzahl_seiten_im_katalog,
                             position_auf_seite,
                           ...
                           LEFT JOIN
                             reisen_reisekategorien
                           ON
                             reisen_reise.reiseID = reisen_reisekategorien.reise_id_id
                           LEFT JOIN
                             reisen_kategorie
                           ON
                             reisen_reisekategorien.kategorie_id_id = reisen_kategorie.kategorieID
                           LEFT JOIN
                             (
                               SELECT
                                 reisen_reisepreise.reise_id_id,
                                 GROUP_CONCAT(
                                   CONCAT(
                                     hauptpreis.titel,
                                     ': ',
                                     IF(reisen_reisepreise.kommentar = 'ab', 'ab ', ''),
                                     CONCAT(REPLACE(FORMAT(reisen_reisepreise.preis, 0),',','.'), ' €'),
                                     reisen_reisepreise.markierung,
                                     IF(reisen_reisepreise.kommentar = 'ab', ', ', IF(reisen_reisepreise.kommentar = '', ', ', CONCAT(', ', reisen_reisepreise.kommentar, ', '))),
                                     IFNULL(subpreise.zpreis,'')
                                   )
                                 ORDER BY
                                   reisen_reisepreise.position
                                 ASC
                                 SEPARATOR
                                   ', '
                                 ) AS preise
                               FROM
                                 reisen_reisepreise
                               LEFT JOIN
                                 reisen_preis AS hauptpreis
                               ON
                                 hauptpreis.preisID = reisen_reisepreise.preis_id_id
                               LEFT JOIN
                                 (
                                   SELECT
                                     reisen_reisepreiszusatz.reisepreis_id_id,
                                     GROUP_CONCAT(
                                       IF(reisen_reisepreiszusatz.preis > 0, CONCAT(CONCAT_WS(': ', subpreis.titel, replace(FORMAT(reisen_reisepreiszusatz.preis, 0),',','.')), ' €'),  subpreis.titel)
                                     ORDER BY
                                       reisen_reisepreiszusatz.position
                                     ASC
                                     SEPARATOR
                                     ', '
                                     ) AS zpreis
                                   FROM
                                     reisen_reisepreiszusatz
                                   LEFT JOIN
                                     reisen_preis AS subpreis
                                   ON
                                     subpreis.preisID = reisen_reisepreiszusatz.preis_id_id
                                   GROUP BY
                                     reisen_reisepreiszusatz.reisepreis_id_id
                                 ) AS subpreise
                               ON
                                 reisen_reisepreise.reisepreisID = subpreise.reisepreis_id_id
                               GROUP BY
                                 reisen_reisepreise.reise_id_id
                             ) AS RP
                           ON
                           ...
                           AND
                             DATE_FORMAT(reisen_reisetermine.datum_beginn, '%Y') > 2025
                           AND
                             reisen_kategorie.kategorie IN ('Wanderreisen', 'kombinierte Flug- & Busreisen', 'Flusskreuzfahrten')
                           ORDER BY
                             reisen_kategorie.kategorie,
                             Seite
                         )
                         ...
                         (
                           SELECT DISTINCT
                             ...
                             reisen_reise.individualbuchbar,
                             reisen_reise.neu,
                             'Zielregion' AS kategorietyp,
                             reisen_zielregion.name,
                             reisen_reise.korrektur_bemerkung_intern AS intern,
                             reisen_reise.titel AS Reise,
                             (TO_DAYS(reisen_reisetermine.datum_ende)-TO_DAYS(reisen_reisetermine.datum_beginn)+1) AS Tage,
                             katalogseite,
                           ...
                           ON
                             reisen_reise.reiseID = reisen_reisekatalogzugehoerigkeit.reise_id_id
                           LEFT JOIN
                             reisen_reisezielregionen
                           ON
                             reisen_reise.reiseID = reisen_reisezielregionen.reise_id_id
                           LEFT JOIN
                             reisen_zielregion
                           ON
                             reisen_reisezielregionen.zielregion_id_id = reisen_zielregion.zielregionID
                           LEFT JOIN
                             (
                               SELECT
                                 reisen_reisepreise.reise_id_id,
...
def reiseterminuebersicht_alles(pk):
    ...
    cursor.execute("select reisen_reise.reiseID as pk, reisen_reise.korrektur_bemerkung_intern as intern, Veranstalter, DAYOFYEAR(datum_beginn) as tmp, 0 as tmp2, 0 as bus, date_format(reisen_reisetermine.datum_beginn, '%M') as Monat, reisen_reise.individualbuchbar, reisen_reise.neu, reisen_reise.titel as Reiseziel, reisen_reisetermine.datum_beginn, reisen_reisetermine.datum_ende, CONCAT(date_format(reisen_reisetermine.datum_beginn,'%d.%m.'), '-', date_format(reisen_reisetermine.datum_ende,'%d.%m.%y')) as Termin, (to_days(reisen_reisetermine.datum_ende)-to_days(reisen_reisetermine.datum_beginn)+1) as Tage, katalogseite, anzahl_seiten_im_katalog, position_auf_seite, katalogseite as Seite from reisen_reise left join reisen_reisetermine on (reisen_reise.reiseID = reisen_reisetermine.reise_id_id) left join reisen_reisekatalogzugehoerigkeit on(reisen_reise.reiseID = reisen_reisekatalogzugehoerigkeit.reise_id_id) where reisen_reisekatalogzugehoerigkeit.katalog_id_id = '" + str(pk) + "' AND date_format(reisen_reisetermine.datum_beginn, '%Y') > 2019 order by datum_beginn, datum_ende, katalogseite, position_auf_seite;")
...
#          katalogseite,
#          IF(
#            anzahl_seiten_im_katalog>1,
#            CONCAT('|',katalogseite+1),
#            ''
#          )
#        ) AS Seite
#      FROM
...
#    """)
    ...
    for i in range(len(termine)):
      ...
      if u'Andalusien' in termine[i].Reiseziel and andalusien != True:
        ...
        termine[i] = termine[i]._replace(Termin = '')
      #if u'Swinemünde' in termine[i].Reiseziel and swine != True:
      #  termine[i] = termine[i]._replace(Termin = 'Okt. 26 bis März 27')
      #  termine[i] = termine[i]._replace(Tage = '8')
      #  swine = True
      #elif u'Swinemünde' in termine[i].Reiseziel and swine != False:
      #  termine[i] = termine[i]._replace(Termin = '')
