                            ...
                            SELECT
                              ...
                              GROUP_CONCAT(
                                CONCAT(
                                  ...
                                  ': ',
                                  IF(
                                    reisen_reisepreise.kommentar = 'ab',
                                    'ab ',
                                    ''
                                  ),
                                  ...
                                  IF(
                                    ...
                                    ', ',
                                    IF(
                                      reisen_reisepreise.kommentar = '',
                                      ', ',
                                      CONCAT(', ', reisen_reisepreise.kommentar, ', ')
                                    )
                                  ),
                                  IFNULL(subpreise.zpreis,'')
                                ) ORDER BY reisen_reisepreise.position ASC SEPARATOR ', '
                              ) AS preise
                            FROM
                              reisen_reisepreise
                            LEFT JOIN
                            ...
                            LEFT JOIN
                              (
                                SELECT
                                  ...
                                  GROUP_CONCAT(
                                    IF(
                                      reisen_reisepreiszusatz.preis > 0,
                                      CONCAT(
                                        CONCAT_WS(
                                          ': ',
                                          subpreis.titel,
                                          REPLACE(FORMAT(reisen_reisepreiszusatz.preis, 0),',','.')
                                        ),
                                        ' €'
                                      ),
                                      subpreis.titel
                                    ) BY reisen_reisepreiszusatz.position ASC SEPARATOR ', '
                                  ) AS zpreis
                                FROM
                                  reisen_reisepreiszusatz
                                LEFT JOIN
                          ...
                          (
                            SELECT
                              ...
                              GROUP_CONCAT(
                                ...
                                ORDER BY
                                  reisen_reisepreise.position ASC SEPARATOR ', '
                              ) AS preise
                            FROM
                              reisen_reisepreise
                            LEFT JOIN
                                ...
                                SELECT
                                  ...
                                  GROUP_CONCAT(
                                    IF(
                                      reisen_reisepreiszusatz.preis > 0,
                                      CONCAT(
                                        CONCAT_WS(': ', subpreis.titel, replace(FORMAT(reisen_reisepreiszusatz.preis, 0),',','.')),
                                        ' €'
                                      ),
                                      subpreis.titel
                                    )
                                    ORDER BY reisen_reisepreiszusatz.position ASC SEPARATOR ', '
                                  ) AS zpreis
                                FROM
                                  reisen_reisepreiszusatz
                                LEFT JOIN
...
def sommer25(request):
    ...
    kategorien = [ u'Wanderreisen', u'kombinierte Flug- & Busreisen', u'Kuren, Gesundheits- und Wellnessreisen', u'Flusskreuzfahrten']
    zielregionen = [ u'Deutschland', u'Österreich', u'Slowenien / Kroatien', u'Schweiz', u'Italien', u'Frankreich', u'Britische Inseln', u'Holland / Belgien', u'Skandinavien / Baltikum', u'Polen / Tschechien']
...
def reiseuebersichtwinter_alles(pk):
    ...
    for i in range(len(termine)):
      ...
      if u'Azoren' in termine[i].Reiseziel and azoren != True:
        ...
        termine[i] = termine[i]._replace(Termin = '')
      if u'ewige Stadt' in termine[i].Reiseziel and rom != True:
        termine[i] = termine[i]._replace(Termin = 'Okt. 24 & Okt. 25')
        rom = True
      elif u'ewige Stadt' in termine[i].Reiseziel and rom != False:
        termine[i] = termine[i]._replace(Termin = '')
      if u'Andalusien' in termine[i].Reiseziel and andalusien != True:
